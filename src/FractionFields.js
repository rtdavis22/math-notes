import {Brace as B, Math as m} from './math/Math.js';
...
function FractionFields() {
  return (
	...
	If there is an injection from \(R\) to another field \(F'\), then there is an injection from \(F\) to \(F'\) which sends \(<m.frac><>a</><>b</></m.frac>\) to \(ab^<B>-1</B>\).
